...
import com.csio.hexagonal.infrastructure.rest.request.DprCbsAccountInfoRequest;
...
public class CorpibProcedureHandler {
    ...
    private final QueryUseCase<DprCbsAccountInfoQuery, DprCbsAccountInfoResponse> dprCbsAccountInfoUseCase;
    private final Executor virtualExecutor;

    public CorpibProcedureHandler(
            QueryUseCase<DprSrcActInfoQuery, DprSrcActInfoResponse> dprSrcActInfoUseCase,
            QueryUseCase<DprCbsAccountInfoQuery, DprCbsAccountInfoResponse> dprCbsAccountInfoUseCase,
            @Qualifier("virtualExecutor") Executor virtualExecutor
    ) {
        this.dprSrcActInfoUseCase = dprSrcActInfoUseCase;
        this.dprCbsAccountInfoUseCase = dprCbsAccountInfoUseCase;
    ...
    @Operation(
            summary = CorpibSpec.DPR_CBS_ACCOUNT_INFO_SUMMARY,
            description = CorpibSpec.DPR_CBS_ACCOUNT_INFO_DESCRIPTION
    )
    @RequestBody(
            description = CorpibSpec.DPR_CBS_ACCOUNT_INFO_DESCRIPTION,
            required = true,
            content = @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = DprCbsAccountInfoRequest.class),
                    examples = @ExampleObject(
                            name = CorpibSpec.DPR_CBS_ACCOUNT_INFO_EXAMPLE_NAME,
                            value = CorpibSpec.DPR_CBS_ACCOUNT_INFO_EXAMPLE_VALUE
                    )
            )
    )
    public Mono<ServerResponse> dprCbsAccountInfo(ServerRequest request) {
        String token = request.headers().firstHeader("Authorization");

        return request.bodyToMono(DprCbsAccountInfoRequest.class)
                .map(req -> new DprCbsAccountInfoQuery(req.actNum()))
                .flatMap(query -> dprCbsAccountInfoUseCase.query(query, token)
                        .subscribeOn(Schedulers.fromExecutor(virtualExecutor)))
                .map(ResponseHelper::success)
                .flatMap(wrapper -> ServerResponse.ok()
                        .contentType(MediaType.APPLICATION_JSON)
